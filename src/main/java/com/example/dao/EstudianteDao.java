...
public interface EstudianteDao extends JpaRepository<Estudiante, Integer> {
    ...
    @Query("SELECT c.id, c.descripcion, c.horario, GROUP_CONCAT(e.id) FROM Estudiante e JOIN e.curso c GROUP BY c.id")
    ...
    List<Map<Curso, List<Estudiante>>> obtenerEstudiantesAgrupadosPorCurso();
